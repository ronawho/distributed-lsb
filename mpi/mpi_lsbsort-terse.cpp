// this is a version of mpi_lsbsort.cpp with comments, printing code,
...
void myMpiAllToAllV(const std::vector<T>& sendBuf,
  ...
  MPI_Alltoallv(&sendCounts[0], &tmpCounts[0], &tmpDspl[0], MPI_INT,
                &recvCounts[0], &tmpCounts[0], &tmpDspl[0], MPI_INT,
                MPI_COMM_WORLD);
...
void exclusiveScan(const DistributedArray<int64_t>& Src,
  ...
  MPI_Exscan(&myTotal, &myGlobalStart, 1, MPI_LONG_LONG, MPI_SUM,
             MPI_COMM_WORLD);
...
int main(int argc, char *argv[]) {
  ...
  MPI_Type_contiguous(3, MPI_UNSIGNED_LONG_LONG,
                      &gShuffleBufSortElementMpiType);
